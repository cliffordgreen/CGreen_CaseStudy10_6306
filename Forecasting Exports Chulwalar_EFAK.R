...
ModelWithoutTrendAndSeasonality <- tslm(EfakAsIs ~ CEPI + SIExtern + UrbanoExports + GlobalisationPartyMembers + AEPI)
...
ModelWithHighCorrelatingIndicators_2012 <- tslm(EfakAsIsVector_2012 ~ trend + season + CEPI_2012 + SIExtern_2012 + UrbanoExports_2012 + GlobalisationPartyMembers_2012 + AEPI_2012)
...
cor(ModelWithHighCorrelatingIndicators_PointForecast, EfakAsIsVector_2013) 
cor(EfakAsIsVector_2013, TotalPlan_2013)

# A Comparison with linear regression also supports the result.
ModelWithHighCorrelatingIndicators_forecast_lm <- lm(EfakAsIsVector_2013 ~ ModelWithHighCorrelatingIndicators_PointForecast, data = EfakAsIsVector_2013)
EfakAsIsVector_2013_lm <- lm(EfakAsIs_2013 ~ EfakAsIs_2013, data = EfakAsIs_2013)
...
adf.test(EfakAsIs, alternative = "stationary")
...
ns <- nsdiffs(EfakAsIs)
if(ns > 0) {
  xstar <- diff(EfakAsIs,lag=frequency(EfakAsIs),differences=ns)
} else {
  xstar <- EfakAsIs
}
nd <- ndiffs(xstar)
...
ChulwalarDiff_lag <- diff(EfakAsIs,lag=12)
...
summary(Model_auto.arima_forecast)
...
Model_dynreg <- Arima(EfakAsIs, xreg=CEPI_lagged, order=c(2,2,0))
...
EfakAsIsVector_2014 <- c(ImportedAsIsData[2:13,8])
EfakAsIsVector_2014
EfakAsIs_2014 <- ts(EfakAsIsVector_2014, start=c(2014,1), end=c(2014,12), frequency=12)
EfakAsIs_2014
cor(EfakAsIs_2014,Pointforecast_ModelWithInflationAndNationalHolidays_2014)
...
cor(EfakAsIs_2014,EfakPlan_2014)
# 0.9448221
cor(EfakAsIs_2014,PointForecast_ModelWithLowCorrelatingIndicators_2014)
# 0.9178468
cor(EfakAsIs_2014,PointForecast_TrendAndSeasonality_2014)
# 0.9349765
cor(EfakAsIs_2014,Model_ets_PointForecast)
# 0.9311129
cor(EfakAsIs_2014,Model_auto.arima_PointForecast)
# 0.9238591
cor(EfakAsIs_2014,Model_dynreg_auto.arima_PointForecast)
# 0.9259971
cor(EfakAsIs_2014,Pointforecast_ModelWithInflationAndNationalHolidays_2014)
# 0.917632
...
# Percentual differences
 ...
 MAPE_AsIs_Plan <- mean(abs(EfakAsIs_2014-EfakPlan_2014)/(EfakAsIs_2014))*100
 MAPE_AsIs_LowCorInd <- mean(abs(EfakAsIs_2014-PointForecast_ModelWithLowCorrelatingIndicators_2014)/(EfakAsIs_2014))*100
 MAPE_AsIs_TrendSeasonality <- mean(abs(EfakAsIs_2014-PointForecast_TrendAndSeasonality_2014)/(EfakAsIs_2014))*100
 MAPE_AsIs_ETSHoltWinters <- mean(abs(EfakAsIs_2014-Model_ets_PointForecast)/(EfakAsIs_2014))*100
 MAPE_AsIs_ARIMA <- mean(abs(EfakAsIs_2014-Model_auto.arima_PointForecast)/(EfakAsIs_2014))*100
 MAPE_AsIs_DynamicReg <- mean(abs(EfakAsIs_2014-Model_dynreg_auto.arima_PointForecast)/(EfakAsIs_2014))*100
 MAPE_AsIs_Kappa <- mean(abs(EfakAsIs_2014-Pointforecast_ModelWithInflationAndNationalHolidays_2014)/(EfakAsIs_2014))*100
