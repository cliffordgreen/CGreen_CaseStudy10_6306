...
Looking at all the AIC values for the models above it seems like the the lowest is AEPI followed by CEPI. Looking at the plot its clear that model somewhat follows the Efak trend. So when we begin forecasting we will know where to start
...
```
The AIC value of 1554 is still higher than the total Urbano Exports model
